...
- `coach_notes`: the coach's write-up for that day, copied VERBATIM and COMPLETE — every paragraph, original wording, paragraphs separated by a blank line. Do not summarize, trim, or paraphrase. If a track's commentary is written once for the whole week (e.g. the ATHX intro + "ATHX PROGRAMMING" paragraph), put that full text on each of that track's days. Empty string if there is none.
- `cycle.full_text`: the complete cycle / programming write-up (e.g. the "NEW CYCLE…" section), verbatim, when present.
...
- `stimulus` per section (see schema rule 8): a list of {quality, regions?, minutes} whose minutes sum to est_minutes. Lifting qualities: power / strength / hypertrophy / skill, each with `regions` from quads, posterior chain, upper push, upper pull, core, arms, full body. Conditioning qualities: aerobic / threshold / vo2 / hiit / sprint, no regions. Classify by rep range, load and time domain — e.g. "5 sets of 2-2-1 building" = strength; "3 × 10–12 @ 31X1" = hypertrophy; "1RM/3RM/5RM tests" = strength; power snatch = power (full body); pull-up/C2B/BMU practice = skill (upper pull); 8-min AMRAP = threshold; E2M × 9 = hiit; 4-mile-run hybrid = aerobic. Omit `stimulus` on mobility/flow sections.
